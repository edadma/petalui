...
const ContextMenuDividerComponent: React.FC<ContextMenuDividerProps> = ({ className = '' }) => {
  return <li className={`d-divider my-1 ${className}`}></li>
}
...
const ContextMenuSubMenuComponent: React.FC<ContextMenuSubMenuProps> = ({
  ...
  return (
    ...
    >
      ...
      {showSubmenu && (
        <ul
          className="d-menu bg-base-100 d-rounded-box shadow-lg border border-base-300 absolute left-full top-0 min-w-[160px] z-50 p-1"
          onMouseEnter={handleMouseEnter}
          onMouseLeave={handleMouseLeave}
        >
          {children}
...
const MenuItem: React.FC<{
  ...
  if (item.divider) {
    return <li className="d-divider my-1"></li>
  }
      ...
      {visible &&
        createPortal(
          <ContextMenuContext.Provider value={contextValue}>
            <ul
              ref={menuRef}
              className={`d-menu bg-base-100 d-rounded-box shadow-lg border border-base-300 min-w-[160px] p-1 fixed z-[9999] ${className}`}
